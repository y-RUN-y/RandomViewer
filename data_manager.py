...
cursor.execute('CREATE TABLE IF NOT EXISTS images (id INTEGER PRIMARY KEY, path TEXT, viewed BOOLEAN)')
...
def get_random_unviewed_image():
    cursor.execute('SELECT * FROM images WHERE viewed = ? ORDER BY RANDOM() LIMIT 1', (False,))
